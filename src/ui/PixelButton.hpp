...
private:
    ...
    void addCluster(int x, int y)
    {
        ...
        float n = fmodf(sinf(x * 12.9898f + y * 78.233f) * 43758.5453f, 1.0f);

        if (n > 0.6f)
            _cells.push_back({x + 1, y});
        if (n > 0.7f)
            _cells.push_back({x, y + 1});
        if (n > 0.8f)
            _cells.push_back({x + 1, y + 1});
    }
    ...
    void generateCells(void)
    {
        ...
        for (int y = -2; y <= rows + 1; y++)
        {
            ...
            {
                bool outside = x < 0 || x >= cols || y < 0 || y >= rows;

                if (!outside)
                    continue;

                float noise = fmodf(sinf(x * 15.73f + y * 91.17f) * 43758.5453f, 1.0f);
...
public:
    void drawDefault(void) override
    {
        UIButton::drawDefault();

        Rectangle r = rect();

        Color c = hover() ? Color{255, 220, 120, 100} : Color{0, 0, 0, 80};
    ...
    PixelButton(const std::string &text, Rectangle rect) : UIButton(text, rect)
    {
        generateCells();
    }
};